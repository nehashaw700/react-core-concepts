...
authRouter.use("/admin", adminAuth);
...
authRouter.post("/signup", async (req, res) => {
  try {
    ...
    res
      .status(400)
      .send("Something went wrong! Not able to add " + error.message);
  }
});

// added rate limiter to login api
  ...
  try {
    ...
    if (isPasswordValid) {
      ...
      res.cookie("token", token, {
        httpOnly: true,
        secure: true,
        sameSite: "strict", // Cookie only sent from sameSite where it was stored.
      });
